...
const allProducts = [
    ...
    },
    {
        ...
        variations: [
            ...
            { name: "1 Kg", price_modifier: 0, base_price: 15000},
        ],
        category: "Buah"
    },
    {
...
// Ensure updateCartCount is defined globally if cart.js is not loaded
// js/script.js
// ... (allProducts array remains the same) ...

document.addEventListener('DOMContentLoaded', () => {
    const productList = document.getElementById('product-list');
    // Remove mobileMenuButton, mobileMenu, currentYearSpan, currentYearFooterSpan
    // as these are handled by global.js now.

    const searchInputDesktop = document.getElementById('search-input-desktop'); // Use ID from _navbar.html
    const searchInputMobile = document.getElementById('search-input-mobile');   // Use ID from _navbar.html
    const searchMessageEl = document.getElementById('search-message'); // Corrected variable name
    const noProductsMessage = document.getElementById('no-products-message');

    // Check if on index page before trying to access productList
    if (window.location.pathname.endsWith('/') || window.location.pathname.endsWith('index.html')) {
        if (productList) {
            displayProducts(allProducts); // Initial display
        }

        // Handle search query from URL (e.g., after redirect from another page)
    ...
    // Expose filter function globally for global.js to use
    ...
    function displayProducts(productsToDisplay) {
        ...
        productsToDisplay.forEach(product => {
            const priceToDisplay = product.variations?.find(v => v.is_default)?.base_price ?? product.price;
            const defaultVariation = product.variations?.find(v => v.is_default) ?? (product.variations?.[0] ?? { name: product.unit });
            const unitToDisplay = defaultVariation.name; // Show full variation name as unit

            const card = `
                <div class="product-card">
                    <a href="product-detail.html?id=${product.id}" class="block" data-no-transition>
                        <img src="${product.image || 'https://via.placeholder.com/300x200.png?text=No+Image'}" alt="${product.name}">
                    </a>
                    <div class="card-content">
                        ...
                        <p class="card-description">${product.description.substring(0, 70)}...</p>
                        <div class="card-actions">
                            <button onclick="addToCart(${product.id}, '${product.name}', ${priceToDisplay}, '${product.imageThumb || product.image}', 1, '${defaultVariation.name}')" class="btn btn-primary w-full">
                                <i class="fas fa-cart-plus mr-2"></i>Tambah
                            </button>
                            <a href="product-detail.html?id=${product.id}" class="mt-2 block text-center primary-text hover:underline text-sm" data-no-transition>Lihat Detail</a>
                        </div>
                    </div>
                </div>
            `;
    ...
    window.displayProducts = displayProducts; // Expose for direct call from no-products message
...
window.allProducts = allProducts; // Keep this global

// addToCart and updateCartCount should primarily live in cart.js and be made global there.
// Remove placeholder addToCart from here if cart.js is always loaded.

// js/script.js
// ... (allProducts array tetap sama) ...
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname.endsWith('/') || window.location.pathname.endsWith('index.html')) {
        if (productList) {
            displayProducts(allProducts); // Tampilan awal
        }
        ...
        if (searchQueryFromUrl) {
            ...
            if (searchInputMobile) searchInputMobile.value = searchQueryFromUrl;
            filterProductsBySearch(searchQueryFromUrl);
             // Gulir ke bagian produk jika ada query pencarian dari URL
    ...
    window.filterProductsBySearch = filterProductsBySearch; // Ekspos fungsi filter

    // Fungsi ini sekarang akan menjadi bagian dari window scope karena pemanggilan dari onclick di no-products-message
    window.displayProducts = function(productsToDisplay) {
        if (!productList) return;
        productList.innerHTML = '';
        ...
        productsToDisplay.forEach(product => {
            const priceToDisplay = product.variations?.find(v => v.is_default)?.base_price ?? product.price;
            const defaultVariation = product.variations?.find(v => v.is_default) ?? (product.variations?.[0] ?? { name: product.unit });
            const unitToDisplay = defaultVariation.name;
            ...
            const card = `
                <div class="product-card">
                    ...
                    <div class="card-content">
                        ...
                        <p class="card-description">${product.description.substring(0, 70)}...</p>
                        <div class="card-actions">
                            <button onclick="addToCart(${product.id}, '${product.name.replace(/'/g, "\\'")}', ${priceToDisplay}, '${(product.imageThumb || product.image).replace(/'/g, "\\'")}', 1, '${defaultVariation.name.replace(/'/g, "\\'")}')" class="btn btn-primary">
                                <i class="fas fa-cart-plus mr-2"></i>Tambah
                            </button>
                            <a href="product-detail.html?id=${product.id}" class="detail-link" data-no-transition>Lihat Detail</a>
                        </div>
                    </div>
                </div>
            `;
    ...
    function filterProductsBySearch(searchTerm) {
        ...
        displayProducts(filtered); // Panggil displayProducts yang sudah di window scope
    }
});

window.allProducts = allProducts; // Tetap global